...
exports.migrateToLeagueMaster = migrateToLeagueMaster;
exports.migrateSingleLeague = migrateSingleLeague;
const firebase_1 = require("../src/firebase");
async function migrateToLeagueMaster({ season }) {
    const db = await (0, firebase_1.getDb)();
    const stats = {
        leaguesProcessed: 0,
    ...
    try {
        ...
        const leagueMasterMap = new Map(); // key -> leagueMasterId
...
async function migrateSingleLeague({ leagueId, season, }) {
    const db = await (0, firebase_1.getDb)();
    const stats = {
