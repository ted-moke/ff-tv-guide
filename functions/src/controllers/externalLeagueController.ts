...
const FLEAFLICKER_API_URL = 'https://www.fleaflicker.com/api/FetchUserLeagues?sport=NFL&email=';
...
export const getExternalLeagues = async (req: Request, res: Response) => {
  ...
  try {
    const credentialDoc = await db.collection('platformCredentials').doc(credentialId as string).get();
