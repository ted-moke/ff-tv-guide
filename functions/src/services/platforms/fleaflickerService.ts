...
export class FleaflickerService {
  async upsertLeague({ leagueName, externalLeagueId, platformCredentialId }: { leagueName: string, externalLeagueId: string, platformCredentialId: string }): Promise<League> {
  ...
  async upsertTeams(league: League) {
    ...
    const matchups = await this.fetchMatchups(league.externalLeagueId, week, season);
    const teamsCollection = db.collection('teams');

    for (const matchup of matchups) {
      for (const teamData of matchup.teams) {
        const rosterData = await this.fetchRoster(league.externalLeagueId, teamData.id, season, week);
        const team: Team = {
          ...
          opponentId: matchup.teams.find(t => t.id !== teamData.id)?.id || '',
          playerData: this.processPlayerData(rosterData.groups),
        };

        await teamsCollection.doc(team.externalTeamId).set(team, { merge: true });
      }
    }
  }

  async upsertUserTeams({league, externalUserId, userId}: { league: League, externalUserId: string, userId: string}) {
    console.log('Upserting userTeams')
    const teamsCollection = db.collection('teams');
    const userTeamsCollection = db.collection('userTeams');

    const teamsQuery = await teamsCollection.where('leagueId', '==', league.id).get();
  ...
  private async fetchMatchups(externalLeagueId: string, week: number, season: number): Promise<any> {
    return new Promise((resolve, reject) => {
      https.get(`https://www.fleaflicker.com/api/FetchLeagueScoreboard?league_id=${externalLeagueId}&season=${season}&scoring_period=${week}`, (res) => {
        let data = '';
  ...
  private async fetchRoster(externalLeagueId: string, externalTeamId: string, season: number, week: number): Promise<any> {
    return new Promise((resolve, reject) => {
      https.get(`https://www.fleaflicker.com/api/FetchRoster?sport=NFL&league_id=${externalLeagueId}&team_id=${externalTeamId}&season=${season}&scoring_period=${week}`, (res) => {
        let data = '';
        res.on('data', (chunk) => data += chunk);
  ...
  private processPlayerData(groups: any[]): Player[] {
    return groups.flatMap(group => 
      group.slots.map(slot => ({
        ...
        logicalName: slot.player.proPlayer.nameFull.toLowerCase().replace(/\s/g, ''),
